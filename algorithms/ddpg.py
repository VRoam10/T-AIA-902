...
class Actor(nn.Module):
    """Deterministic policy: state -> action preferences in [-1, 1]."""
...
class DDPGAgent(BaseAgent):
    ...
    Supports both discrete-state (e.g. Taxi) and continuous-state (e.g. BeamNG)
    environments. Always outputs discrete actions (argmax of actor output)
    since current environments use discrete action spaces.
    """
    ...
    def __init__(
        ...
        net_input_size = n_states  # one-hot width for discrete, vector size for continuous

        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        print(f"[DDPGAgent] Using device: {self.device}  (discrete_states={self.discrete_states})")

        # Actor (policy)
        self.actor = Actor(net_input_size, n_actions).to(self.device)
        self.actor_target = copy.deepcopy(self.actor)
        self.actor_optimizer = optim.Adam(self.actor.parameters(), lr=actor_lr)

        # Critic (Q-value)
        self.critic = Critic(net_input_size, n_actions).to(self.device)
        self.critic_target = copy.deepcopy(self.critic)
        self.critic_optimizer = optim.Adam(self.critic.parameters(), lr=critic_lr)

        # Exploration noise
        self.noise = OUNoise(n_actions, theta=noise_theta, sigma=noise_sigma)
    ...
    def select_action(self, state) -> int:
        ...
        # Add scaled OU noise for exploration (epsilon controls noise magnitude)
        if self._epsilon > 0:
            noise = self.noise.sample() * self._epsilon
            action = np.clip(action + noise, -1.0, 1.0)

        # Treat actor outputs as action preferences -> pick the best one
    ...
    def load(self, path: str = "ddpg_model.pth"):
        ...
        print(f"[DDPGAgent] Loaded <- {path}  (eps={self._epsilon:.3f}, steps={self.train_steps})")
    ...
    def _encode_state(self, state) -> torch.Tensor:
        """Convert a state to a tensor suitable for the networks."""
    ...
    def _encode_state_np(self, state) -> np.ndarray:
        """Encode state as a numpy array for the replay buffer."""
        if self.discrete_states:
            one_hot = np.zeros(self.n_states, dtype=np.float32)
            one_hot[int(state)] = 1.0
            return one_hot
    ...
    def _encode_action_np(self, action) -> np.ndarray:
        """One-hot encode a discrete action for the critic network."""
